...
class AuthProvider extends ChangeNotifier {
  ...
  /// Register a new account via ApiService
  Future<void> signup({
    ...
    try {
      ...
      // Fallback to local auth if backend fails or isn't running yet
      debugPrint('Backend registration failed ($e), falling back to local auth.');
    }
  ...
  /// Sign in with email + password via ApiService.
  Future<void> login({
    required String email,
    required String password,
  }) async {
    ...
    try {
      ...
      _role = (data['role'] ?? '') == 'caregiver' ? UserRole.caregiver : UserRole.patient;
    } catch (e) {
      // Fallback to local auth if backend fails or isn't running yet
      debugPrint('Backend login failed ($e), falling back to local auth.');
      final prefs = await SharedPreferences.getInstance();
