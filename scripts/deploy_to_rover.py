...
rover_ip_val = "10.34.19.247"

if os.path.exists(env_path):
    try:
        with open(env_path, "r") as f:
            for line in f:
                if line.strip().startswith("ROBOT_IP="):
                    rover_ip_val = line.strip().split("=")[1].strip()
                    break
    except Exception:
        pass

ROVER_IP = rover_ip_val
ROVER_USER = "root"
ROVER_PASS = "0000"
...
def main():
    ...
    with SCPClient(ssh.get_transport(), progress=progress) as scp_client:
        ...
        if os.path.exists(robot_service_dir):
            ...
            print("[WARN] services/robot directory not found, skipping.")
        ...
        if os.path.exists(runtime_dir):
            for root, dirs, files in os.walk(runtime_dir):
                if "__pycache__" in root or ".git" in root or "robot_ui" in root:
                    continue
            ...
            print("[WARN] optimized_runtime directory not found, skipping.")
