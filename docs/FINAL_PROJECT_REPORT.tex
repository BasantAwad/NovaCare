...
\tightlist
\item
  \textbf{Contextual Awareness}: The LLM module doesn't just respond to
  text; it analyzes the sentiment and emotional tone of the user's
  input.
\item
  \textbf{State Emitting}: Based on the conversation, the LLM emits
  emotional states (e.g., \texttt{happy}, \texttt{concerned},
  \texttt{thinking}, \texttt{listening}). This emotional state is then
  broadcasted to the Robot UI to physically change the robot's animated
  facial expressions, bridging the gap between digital AI and physical
  empathy.
\end{itemize}
