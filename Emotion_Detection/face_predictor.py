...
# ============================================================================
# Testing / Demo - Live Webcam
# ============================================================================
if __name__ == "__main__":
    print("=" * 60)
    print("Face Emotion Analyzer - Live Webcam Demo")
    print("=" * 60)
    
    # Initialize analyzer with HuggingFace model
    analyzer = FaceEmotionAnalyzer(use_huggingface=True)
    
    if not HF_AVAILABLE or analyzer.model is None:
        print("\nError: Model not loaded. Please install transformers:")
        ...
        if not cap.isOpened():
            print("Error: Could not open webcam. Check if camera is available.")
        else:
            frame_count = 0
            while True:
                ret, frame = cap.read()
                ...
                result = analyzer.predict(frame, detect_face=True, return_all_scores=False)
                emotion = result['emotion']
                confidence = result['confidence']
                face_detected = result.get('face_detected', False)
                
                # Display info on frame
                text = f"Emotion: {emotion} ({confidence:.2%})"
                ...
                frame_count += 1
                if frame_count % 10 == 0:
                    print(f"Frame {frame_count}: {emotion} ({confidence:.2%}) - Face: {face_detected}")
