...
class SlangExpander:
    ...
    SLANG_DICT = {
        ...
        'its over':' it is over',
        'i cant handle this':' I cannot handle this I am very anxious',
        ...
        'rizz':' I have charm and am attractive',
        'drip':' I have style and attractiveness',
...
# ============================================================================
# Testing / Demo - Live User Input
# ============================================================================
if __name__ == "__main__":
    print("=" * 60)
    print("Text Emotion Analyzer - Live User Input Demo")
    print("=" * 60)
    
    # Initialize analyzer
    analyzer = TextEmotionAnalyzer(use_pretrained=True)
    
    print("\nLive Text Emotion Analysis")
    print("-" * 60)
    ...
    while True:
        ...
        print(f"\n  → Emotion: {result['emotion']} ({result['fine_grained_emotion']})")
        print(f"  → Confidence: {result['confidence']:.2%}")
        
        if 'all_scores' in result:
            print(f"\n  → All emotion scores:")
            for emotion, score in sorted(result['all_scores'].items(), key=lambda x: -x[1])[:5]:
                print(f"      {emotion}: {score:.2%}")
        
        if 'overall_sentiment' in result:
            print(f"\n  → Sentiment: {result['overall_sentiment'].upper()}")
            print(f"      Positive: {result['sentiment_scores']['positive']:.2%}")
            print(f"      Negative: {result['sentiment_scores']['negative']:.2%}")
            print(f"      Neutral: {result['sentiment_scores']['neutral']:.2%}")
